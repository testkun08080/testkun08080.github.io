...
const AboutTools = () => {
  ...
  return (
    <div style={{
      ...
      <div className="font-semibold text-2xl text-black mb-2 text-left">{t('about_tools_title')}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 18, justifyContent: 'center', alignItems: 'center' }}>
        {tools.map(tool => (
          <div key={tool.name} title={tool.name} style={{ width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {tool.icon ? (
              <img src={tool.icon} alt={tool.name} style={{ width: 34, height: 34 }} />
            ) : tool.devicon ? (
